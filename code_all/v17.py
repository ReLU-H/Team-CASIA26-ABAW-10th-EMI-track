...
def list_common_ids_from_dirs(*dirs, width: int = 5):
    id_sets = []
    for d in dirs:
        ...
        for fn in os.listdir(d):
            if fn.endswith(".pkl"):
                ...
                ids.add(norm_id(stem, width))
        id_sets.append(ids)

    common = set.intersection(*id_sets) if id_sets else set()
    common = sorted(common)
    if len(common) == 0:
        raise RuntimeError("No common .pkl ids found across provided test feature directories.")
    return common


# =========================== dataset ===========================

class EMIMMTAFeatureDatasetV17(Dataset):
    def __init__(
        self,
        df: pd.DataFrame,
        vit_dir: str,
        ...
        if "Filename" not in self.df.columns:
            raise ValueError("Input dataframe must contain column: Filename")

        if self.has_label:
            miss = [c for c in EMO_COLS if c not in self.df.columns]
            if miss:
                raise ValueError(f"Labeled dataset missing columns: {miss}")
    ...
    def __getitem__(self, idx):
        ...
        txt_gap = txt_raw.mean(axis=0).astype(np.float32, copy=False)
        ...
        txt_gap = torch.from_numpy(txt_gap)

        if self.has_label:
            y = torch.tensor(row[EMO_COLS].to_numpy(dtype=np.float32))
            return sid, vit, aud, txt, txt_gap, y
        else:
            return sid, vit, aud, txt, txt_gap
...
def collate_fn_test(batch):
    sids, vit, aud, txt, txt_gap = zip(*batch)
    return (
        list(sids),
        torch.stack(vit, 0),
        torch.stack(aud, 0),
...
class EMI_MMTA_V17(nn.Module):
    def __init__(
        ...
        fusion_in = feat_dim if self.fusion_type == "avg" else feat_dim * 3
...
@torch.no_grad()
def predict_test(model, loader, device, use_amp: bool):
    model.eval()
    all_sids, all_pred = [], []
    amp_device = "cuda" if str(device).startswith("cuda") else "cpu"

    for sids, vit, aud, txt, _txt_gap in tqdm(loader, desc="Infer-Test", ncols=120):
    ...
    return all_sids, np.concatenate(all_pred, axis=0)


def save_model_state(model, save_path: str):
...
def main():
    ...
    # new test feature dirs
    ap.add_argument("--audio_feats_test_set", default="audio_feats_test_set")
    ap.add_argument("--image_feats_test_set", default="image_feats_test_set")
    ap.add_argument("--text_feats_test_set", default="text_feats_test_set")

    # optional test split
    ap.add_argument("--test_split", default="test_split.csv")

    ap.add_argument("--output_dir", default="outputs_emi_mmta_v17")
    ...
    print(f"valid_split           = {args.valid_split}")
    print(f"test_split            = {args.test_split}")
    print(f"audio_dir             = {args.audio_dir}")
    print(f"face_image_dir        = {args.face_image_dir}")
    print(f"wavlm_dir             = {args.wavlm_dir}")
    print(f"audio_feats_test_set  = {args.audio_feats_test_set}")
    print(f"image_feats_test_set  = {args.image_feats_test_set}")
    print(f"text_feats_test_set   = {args.text_feats_test_set}")
    ...
    train_all_df = pd.concat([train_df, valid_df], axis=0, ignore_index=True)

    if os.path.isfile(args.test_split):
        test_df = pd.read_csv(args.test_split)
        if "Filename" not in test_df.columns:
            raise ValueError("test_split csv must contain column: Filename")
        test_df = test_df[["Filename"]].copy()
        print(f"Loaded test split from csv: {args.test_split}, n={len(test_df)}")
    else:
        print(f"[Info] {args.test_split} not found. Building test split from common ids in test feature dirs.")
        test_ids = list_common_ids_from_dirs(
            args.image_feats_test_set,
            args.audio_feats_test_set,
            args.text_feats_test_set,
        )
        test_df = pd.DataFrame({"Filename": test_ids})
        print(f"Built test split from dirs, n={len(test_df)}")

    ds_train = EMIMMTAFeatureDatasetV17(
        train_all_df,
        args.face_image_dir,
        args.audio_dir,
        args.wavlm_dir,
    ...
    ds_test = EMIMMTAFeatureDatasetV17(
        test_df,
        args.image_feats_test_set,
        args.audio_feats_test_set,
        args.text_feats_test_set,
    ...
    print(f"train_all size={len(ds_train)}, test size={len(ds_test)}, seq_len={args.seq_len}")
    ...
    final_ema_path = os.path.join(args.output_dir, "final_ema.pt")
    submission_path = os.path.join(args.output_dir, "submission.csv")

    for ep in range(1, args.epochs + 1):
        model.train()
        pbar = tqdm(dl_train, desc=f"Train {ep}/{args.epochs}", ncols=150)
        running = 0.0
        ...
        sched.step()
        print(f"[Epoch {ep}] avg train loss = {running / max(1, len(dl_train)):.6f}")

    save_model_state(model, final_path)
    print(f"[SAVE] {final_path}")
    if use_ema:
        torch.save(ema.state_dict(), final_ema_path)
        print(f"[SAVE] {final_ema_path}")

    if use_ema and ema is not None:
        with ema_scope(model, ema):
            sids_test, pred_test = predict_test(model, dl_test, device, use_amp)
    else:
        sids_test, pred_test = predict_test(model, dl_test, device, use_amp)

    out = pd.DataFrame({"Filename": sids_test})
    for i, c in enumerate(EMO_COLS):
        out[c] = pred_test[:, i]

    if os.path.isfile(args.test_split):
        ref = pd.read_csv(args.test_split)[["Filename"]].copy()
        ref["Filename"] = ref["Filename"].apply(lambda x: norm_id(x, 5))
        out["Filename"] = out["Filename"].apply(lambda x: norm_id(x, 5))
        out = ref.merge(out, on="Filename", how="left")
        missing = out[EMO_COLS].isna().any(axis=1).sum()
        if missing > 0:
            raise RuntimeError(f"{missing} test samples from test_split were not found in predicted outputs.")
