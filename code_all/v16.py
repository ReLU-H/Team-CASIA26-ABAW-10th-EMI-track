...
class EMI_MMTA_V15_Ablation(nn.Module):
    def __init__(
        ...
        if not (self.use_visual or self.use_text or self.use_audio):
            raise ValueError("At least one of use_visual/use_text/use_audio must be 1.")
        ...
        n_modalities = int(use_visual) + int(use_audio) + int(use_text)
        fusion_in = feat_dim if self.fusion_type == "avg" else feat_dim * n_modalities
    ...
    def forward(self, vit, aud, txt):
        ...
        if self.use_audio:
            ...
            vad = aud.new_full((aud.size(0), 3), 0.5)
        ...
        if self.fusion_type == "avg":
            fused_feat = torch.stack(feats, dim=0).mean(dim=0)
        else:
        ...
        aux_preds = {"visual": pv, "audio": pa, "text": pt}
        return pred, aux_preds, vad, w, fused_feat
...
def main():
    ...
    # modality switches
    ap.add_argument("--use_visual", type=int, default=1)
    ap.add_argument("--use_text", type=int, default=1)
    ap.add_argument("--use_audio", type=int, default=1)
    ...
    if not (bool(args.use_visual) or bool(args.use_text) or bool(args.use_audio)):
        raise ValueError("At least one of --use_visual/--use_text/--use_audio must be 1.")
    ...
    print(f"use_visual         = {args.use_visual}")
    print(f"use_text           = {args.use_text}")
    print(f"use_audio          = {args.use_audio}")
    ...
    n_enabled_modalities = int(bool(args.use_visual)) + int(bool(args.use_audio)) + int(bool(args.use_text))
    ...
    for ep in range(1, args.epochs + 1):
        ...
        for _, vit, aud, txt, txt_gap, y in pbar:
            ...
            with torch.amp.autocast(device_type=amp_device, enabled=use_amp):
                pred, aux_preds, vad, w, fused_feat = model(vit, aud, txt)
                pred = torch.nan_to_num(pred, nan=0.5, posinf=1.0, neginf=0.0).clamp(0.0, 1.0)
                ...
                if bool(args.use_aux_loss):
                    ...
                    aux_weights = {"visual": 0.20, "audio": 0.35, "text": 0.45}
                    for k, wt in aux_weights.items():
                        if aux_preds[k] is not None:
                            l_aux = l_aux + wt * F.smooth_l1_loss(aux_preds[k], y)
                    l_aux = l_aux / max(1, n_enabled_modalities)
                else:
                    l_aux = pred.new_tensor(0.0)

                if bool(args.use_audio) and bool(args.use_vad_module) and bool(args.use_vad_reg):
                    l_vad_reg = ((vad - 0.5) ** 2).mean()
                else:
                    l_vad_reg = pred.new_tensor(0.0)

                if bool(args.use_triplet_module) and bool(args.use_text):
